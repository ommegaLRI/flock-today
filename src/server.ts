...
function sendError(response: ServerResponse, error: unknown, fallbackStatus = 400): void {
...
async function readJsonBody(request: IncomingMessage): Promise<Record<string, unknown>> {
  ...
  if (!contentType.toLowerCase().startsWith('application/json')) {
    throw new FlockProjectError('Flock accepts application/json requests only.', 415, 'invalid_content_type');
  }
  const chunks: Buffer[] = [];
  let size = 0;
  for await (const chunk of request) {
    ...
    if (size > MAX_BODY_BYTES) throw new FlockProjectError('Request body is too large.', 413, 'body_too_large');
    chunks.push(buffer);
  }
  const parsed = JSON.parse(Buffer.concat(chunks).toString('utf8') || '{}') as unknown;
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    throw new FlockProjectError('Request body must be a JSON object.', 400, 'invalid_body');
  }
...
function parseIntent(value: unknown): FlockEditIntent {
  ...
  return {
    goals: Array.isArray(input.goals)
      ? input.goals.filter((goal): goal is string => typeof goal === 'string').slice(0, 5)
      : [],
...
export function installFlockMiddleware(server: ViteDevServer, project: StitchProject, openai: OpenAIInference): void {
  server.middlewares.use(async (request, response, next) => {
    ...
    if (!isLoopback(request)) return sendError(response, new FlockProjectError('Flock is available from localhost only.', 403, 'local_only'));
    if (!sameOrigin(request)) return sendError(response, new FlockProjectError('Cross-origin Flock requests are not allowed.', 403, 'cross_origin'));
    ...
    try {
      ...
      if (request.method === 'GET' && url.pathname.startsWith(`${API_PREFIX}/visual/`)) {
        ...
        if (!visual) throw new FlockProjectError('No Stitch visual exists for this section.', 404, 'visual_missing');
      ...
      if (!route) throw new FlockProjectError('Unknown Flock endpoint.', 404, 'unknown_endpoint');
      ...
      if (request.method === 'POST' && route.action === 'preview') {
        ...
        if (typeof body.baseHash !== 'string' || !body.baseHash) {
          throw new FlockProjectError('baseHash is required.', 400, 'invalid_base_hash');
        }
        if (typeof body.source !== 'string') {
          throw new FlockProjectError('source is required.', 400, 'invalid_source');
        }
      ...
      throw new FlockProjectError('Unknown Flock endpoint.', 404, 'unknown_endpoint');
    } catch (error) {
